function Controller() {
    ...
    this.filterModels = {
        brand: [], color: [], sold_out: []
    };
    this.onDataLoaded = [
        this.initializeFilters, this.renderProducts
    ];
    this.loadData();
}
...
Controller.prototype.loadData = function () {
    $.getJSON(this.url, function (res) {
        console.log(res);
        this.data = res;
        this.onDataLoaded.forEach(function (callback) {
            callback.call(this);
        }.bind(this));
    }.bind(this));
}
...
Controller.prototype.renderProducts = function(){
    var render = this.data.filter(function(product){        
    ...
    render.forEach(function(product){
        this.productsContainer.append(this.getProductTemplate(product));
    }.bind(this));
}
...
Controller.prototype.setUpFilters = function (name) {
    this[name] = this.data.reduce(function (prev, curr, currIndex, array) {
        var result = prev;
        if (currIndex == 1) {
            var result = [];
...
Controller.prototype.createFilterView = function (name) {
    ...
    this[name].forEach(function (filter) {
        var li = $('<li>');
        var checkbox = $('<input type="checkbox">').attr('id', filter).attr('name', name);
        checkbox.click(this.runFilter.bind(this));
        var text = filter == 0
        var label = $('<label>').attr('for', filter).text(this.getFilterText(filter));
        li.append(checkbox, label);
        ul.append(li);        
    }.bind(this));
    this.filterContainer.append(ul);    
}

Controller.prototype.runFilter = function(e){
    if(e.target.checked){
        this.filterModels[e.target.name].push(e.target.id);
    }else{
        var index = this.filterModels[e.target.name].indexOf(e.target.id);
        this.filterModels[e.target.name].splice(index, 1);
    }
